...
export const TOOL_GUIDES: Record<ToolId, ToolGuide> = {
  ...
  peercompare: {
    purpose:
      'Puts your savings rate, expenses, investments and emergency buffer next to a benchmark for your income bracket and city, so the numbers have a scale.',
    steps: [
      'Enter your income, city and the four figures the tool asks for.',
      'Read each metric against its benchmark separately — an overall verdict is less useful than one weak metric.',
      'Start with the largest gap. It is usually the emergency buffer or the savings rate.',
      'Come back after a few months rather than a few days; these metrics move slowly by nature.',
    ],
    method: [
      'Your city sets a cost-of-living multiplier across 14 named Indian cities plus tier-2 and tier-3 groupings, so a Mumbai salary is not compared against an Indore one at face value.',
      'Income places you in a bracket, and the benchmark for each metric is drawn from a calibrated model for that bracket and city tier.',
      'Your figure is expressed as a position against that benchmark, per metric.',
    ],
    worked: {
      title: 'Why the city matters more than the salary',
      rows: [
        { label: 'Income', value: '₹18,00,000 a year, in both cases' },
        { label: 'Monthly expenses', value: '₹75,000, in both cases' },
        { label: 'Case A — city', value: 'Mumbai' },
        { label: 'Case B — city', value: 'Indore' },
        { label: 'Same figures, different verdicts', value: 'A sits near its benchmark; B sits well above it' },
      ],
      conclusion:
        'Identical inputs, two different readings, because the cost-of-living multiplier differs. Comparing a metro salary against a tier-2 benchmark at face value is the thing this tool exists to stop.',
    },
    mistakes: [
      'Reading the overall picture instead of the weakest metric. One thin emergency buffer is more actionable than an average that looks fine.',
      'Checking it monthly. These metrics move slowly; a few months apart is the useful interval, and weekly checking measures noise.',
      'Treating the benchmark as a target. It is a calibration aid — a savings rate is more useful as an absolute than as a rank.',
      'Comparing without entering the city. The multiplier is doing a large share of the work and the default will not match your costs.',
    ],
    limits: [
      'The benchmark is a model, not a survey of FinatriX users. No other person’s data is read to produce your comparison.',
      'It knows nothing about dependants, debt, health, caregiving or where you started, all of which legitimately change what "good" looks like.',
      'Comparison is a calibration tool. Beyond that it is a poor motivator, and a savings rate is more useful as an absolute than as a rank.',
    ],
